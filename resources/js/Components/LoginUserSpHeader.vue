...
<template>
    <header>
        ...
        <div class="flex justify-between bg-white sticky top-0 lg:hidden">
            <Link href="/home"><img class="hover:opacity-50 w-8 my-4 mx-4 sm:w-9 sm:my-5 sm:ml-5 lg:w-10 lg:my-6 lg:ml-6" src="/images/logo.svg"></Link>
        ...
        <nav class="overflow-y-auto overflow-x-hidden fixed top-0 left-0 w-full h-full bg-gray-900/25 lg:hidden" @click.self="showModalMenu(false)" v-show="isOpenModalMenu">
            <div class="text-center bg-white h-auto p-4 shadow-xl rounded-xl mt-10 mx-6">
                <ul class="mt-12 flex flex-col gap-8">
                    <Link class="hover:text-gray-500" href="/home"><li class="text-2xl" v-bind:class="{'font-bold':isSelectedUrlPath('/home') }">ホーム</li></Link>
                    <Link class="hover:text-gray-500" href="/me"><li class="text-2xl" v-bind:class="{'font-bold':isSelectedUrlPath('/me') }">自分の投稿</li></Link>
                    <Link class="hover:text-gray-500" href="/others"><li class="text-2xl" v-bind:class="{'font-bold':isSelectedUrlPath('/others') }">その他</li></Link>
                    <li><Link href="/post"><CustomButton>鳴く</CustomButton></Link></li>
                </ul>

                <footer class="mt-16 mb-2">
                    <ul class="flex flex-col justify-around gap-2">
                        <li><Link class="hover:text-gray-500" href="/terms">利用規約</Link></li>
                        <li><Link class="hover:text-gray-500" href="/privacy">プライバシーポリシー</Link></li>
